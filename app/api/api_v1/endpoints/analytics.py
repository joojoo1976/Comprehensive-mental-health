...
def create_mood_entry(
    *,
    ...
    """
    mood_entry = crud.create_mood_entry(db, user_id=current_user.id, mood_in=mood_in)
    return mood_entry
...
def create_sleep_entry(
    *,
    ...
    إنشاء مدخل نوم جديد
    """
    sleep_entry = crud.create_sleep_entry(db, user_id=current_user.id, sleep_in=sleep_in)
    return sleep_entry
...
def create_activity_entry(
    *,
    ...
    """
    activity_entry = crud.create_activity_entry(db, user_id=current_user.id, activity_in=activity_in)
...
def get_recommendations(
    ...
    """
    recommendations = crud.get_user_recommendations(db, user_id=current_user.id)
    return recommendations

@router.get("/progress", response_model=schemas.ProgressReport)
def get_progress_report(
    period: str = Query("week", description="فترة التقرير: day, week, month, year"),
    db: Session = Depends(deps.get_db),
    current_user: models.User = Depends(get_current_user),
) -> Any:
    """
    ...
    """
    progress_report = crud.get_progress_report(db, user_id=current_user.id, period=period)
...
def get_digital_biomarkers_analysis(
    ...
    """
    analysis = crud.get_digital_biomarkers_analysis(db, user_id=current_user.id)
    return analysis
