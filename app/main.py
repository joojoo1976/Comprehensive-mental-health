...
async def log_request_processing_time(request: Request, call_next):
    ...
    logger.info(f'Request: "{request.method} {request.url.path}" processed in {process_time:.4f} secs - Status: {response.status_code}')
    return response
...
from app.core.locale_middleware import LocaleMiddleware, LocaleConsentMiddleware, LocaleRedirectMiddleware
